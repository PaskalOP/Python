...
# собирающий модуль, находясь перед некоторым кустом. На входе задано количество ягод на
#  каждом кусте. Не обязательно вводить их с клавиатуры, можно задать непосредственно в коде
#  программы
...
berry_numbers = [11, 92, 1, 42, 15, 12, 11, 81]
bush = int(input("Пeред каким кустом стоит модуль "))

if bush == len(berry_numbers):
       sum_berry = berry_numbers [bush-1]+berry_numbers [bush-2] + berry_numbers [0]
else:
    sum_berry = berry_numbers[bush-2]+berry_numbers[bush-1] + berry_numbers [bush]
print(f"Макс. кол-во ягод {sum_berry}, собрано для куста {bush}")