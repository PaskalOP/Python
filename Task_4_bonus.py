...
ivanov, saltikov, sokolov, sidorov, petrov = lst_data
persons = [ivanov, saltikov, sokolov, sidorov, petrov]
# persons_dict = dict()
dict_person_full = dict()
for item in persons:
    n = item.index(":")
    fio = tuple(item[:n].split())
    data = item[n+1:].split()
    index_point = data[0].index(":")
    dict_person = dict ()
    for i in data:
        dict_person [f"{i[:index_point]}"]= (i[index_point+1:])
   
    dict_person_full [f"{fio}"] = dict_person
    

print (dict_person_full)